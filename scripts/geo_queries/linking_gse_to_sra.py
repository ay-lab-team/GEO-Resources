...
meta_fn = os.path.join(outdir, '{}.meta.all_columns.tsv'.format(gse_id))
orig_cols_fn = os.path.join(outdir, '{}.meta.major_columns.original.tsv'.format(gse_id))
renamed_cols_fn = os.path.join(outdir, '{}.meta.major_columns.renamed.tsv'.format(gse_id))
output_files = [meta_fn, orig_cols_fn,  renamed_cols_fn]
output_files = '\n'.join(['    - {}'.format(x) for x in output_files])
print('Writing the output to: \n{}'.format(output_files))
...
for gsm_id, gsm in geo_query.gsms.items():
    ...
    # setting default or full description based on availability
    description = ''
...
orig_cols_fn = os.path.join(outdir, '{}.meta.major_columns.original.tsv'.format(gse_id))
meta[final_cols].to_csv(orig_cols_fn, sep='\t', index=False)
...
renamed_cols_fn = os.path.join(outdir, '{}.meta.major_columns.renamed.tsv'.format(gse_id))
final_df.to_csv(renamed_cols_fn, sep='\t', index=False)
